...
public class DemoUserStorageProvider implements UserStorageProvider,
    ...
    public UserModel getUserById(String id, RealmModel realm) {
        log.infov("\nStorage.getUserById > lookup user by id: realm={0} userId={1}", realm.getId(), id);
    ...
    public UserModel getUserByUsername(String username, RealmModel realm) {
        log.infov("\nStorage.getUserByUsername > lookup user by username: realm={0} username={1}", realm.getId(), username);
        User found = repo.searchForUserByUsernameOrEmail(username);
        log.infov("\nStorage.getUserByUsername > Found: " + found.getEmail());
    ...
    public UserModel addUser(RealmModel realm, String username) {
        return new UserAdapter(session, realm, model, repo.CreateNewUser(username));
    }
